...
List available Kan workspaces using \`kan_list_workspaces\`. Ask the user which workspace this chat should be linked to. Once they choose, call \`link_workspace\` to link it. Then call \`advance_kickstart\` with a note about which workspace was linked.`,
      2: `**Step 2: Topics**
Set up Telegram topics — which topic for task reminders (\`set_reminder_topic\`) and which for Gremlin's Corner (\`set_social_topic\`). The user needs to tell you the topic/thread IDs (right-click a topic → Copy Link, the last number in the URL is the thread ID).
