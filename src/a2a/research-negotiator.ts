...
export async function conductResearch(
  ...
  const sendProgress = (text: string) => {
    api
      ...
      .catch((err: unknown) => console.error("Failed to send progress:", err));
  };
  ...
  const researchPromise = (async () => {
    ...
    const stream = client.sendMessageStream({
      message: {
        ...
        role: "user",
        parts: [{ kind: "text", text: fullQuery }],
        kind: "message",
      },
    });
    ...
    while (steeringRounds < MAX_STEERING_ROUNDS && interimFindings) {
      ...
      if (steering.accept) {
        ...
        const acceptStream = client.sendMessageStream({
          message: {
            messageId: crypto.randomUUID(),
            role: "user",
            parts: [{ kind: "text", text: "compile final report" }],
            kind: "message",
            taskId,
            contextId,
          },
      ...
      const steerStream = client.sendMessageStream({
        message: {
          messageId: crypto.randomUUID(),
          role: "user",
          parts: [{ kind: "text", text: steering.feedback }],
          kind: "message",
          taskId,
          contextId,
        },
